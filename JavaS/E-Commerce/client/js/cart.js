...
const displayCart = () => {
    ...
    modalFooter.innerHTML = `
        <div class="total-price">Total: ${formatoPrecio.format(total)}</div>
    `;
    ...
    if (cart.length > 0) {
        ...
        payButton.addEventListener("click", crearPreferencia);
...
const deleteCartProduct = (id) => {
    const fuondId = cart.findIndex((element) => element.id == id);

    cart.splice(fuondId, 1);
